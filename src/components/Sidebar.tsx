...
export const Sidebar: React.FC<SidebarProps> = ({ filters, onFilterChange, isOpen, onClose, products = [] }) => {
  ...
  const sidebarContent = (
    <div className="space-y-4 sm:space-y-6">
      {/* Categories */}
      <div>
        <h3 className="text-base sm:text-lg font-semibold text-gray-800 dark:text-gray-100 mb-3 sm:mb-4">Categories</h3>
        <div className="space-y-1.5 sm:space-y-2">
          <button
            onClick={() => handleCategoryChange('')}
            className={`w-full text-left px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg transition-colors duration-200 text-sm sm:text-base flex items-center justify-between ${filters.category === ''
            ...
            categoriesWithCounts.map((category) => {
              ...
              return (
                <button
                  ...
                  className={`w-full text-left px-2.5 sm:px-3 py-1.5 sm:py-2 rounded-lg transition-colors duration-200 flex items-center space-x-2 sm:space-x-3 text-sm sm:text-base ${filters.category === category.id
                    ? 'bg-blue-600 text-white'
                    : 'hover:bg-gray-100 dark:hover:bg-gray-700 text-gray-700 dark:text-gray-300'
                    }`}
                >
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div className={`
        fixed lg:static inset-y-0 left-0 z-50 w-72 sm:w-80 bg-white dark:bg-gray-800 border-r border-gray-200 dark:border-gray-700
